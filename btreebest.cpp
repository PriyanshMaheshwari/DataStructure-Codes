...
void red(BTPTR &T,int k){
  ...
  if(x<k){
    while(x!=k){
      ...
      T->ptr[x]->ptr[T->ptr[x]->cnt] = T->ptr[x+1]->ptr[0];
      for(int i=0;i<T->ptr[x+1]->cnt-1;i++){
        T->ptr[x+1]->key[i] = T->ptr[x+1]->key[i+1];
        T->ptr[x+1]->ptr[i] = T->ptr[x+1]->ptr[i+1];
      }
      T->ptr[x]->ptr[T->ptr[x]->cnt-1] = T->ptr[x]->ptr[T->ptr[x]->cnt];
      T->ptr[x+1]->cnt--;
      x++;
    }
  }
    ...
    while(x!=k){
      ...
      T->ptr[x]->ptr[0] = T->ptr[x-1]->ptr[T->ptr[x-1]->cnt];
      T->ptr[x-1]->cnt--;
      x--;
    }
  }
...
void overflow(BTPTR &T,stack<BTPTR> &S,int a[],int x){
  if(S.size() == 0){
    ...
    if(P->cnt < 2*d){
      ...
      S.push(P);return;
        ...
        if(S.size() == 0 || !check(S.top())){
          ...
          vector<pair<BTPTR,int>> v2;
          for(int i=0;i<=P->cnt;i++) if(P->ptr[i])v2.push_back(make_pair(P->ptr[i],P->ptr[i]->key[0]));
          v.push_back(make_pair(N,N->key[0]));
          sort(v.begin(),v.end(),compare);
          for(int i=0;i<v.size();i++) P->ptr[i] = v[i].first;
        }
