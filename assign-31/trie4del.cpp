...
int chi(GTPTR T,int n){
  int cnt=0,x;
  for(int i=0;i<=n;i++){
    if(T->u.ptr[i] != NULL){cnt++;x=i;}
  }
  if(cnt > 1) return -1;
  else return x;
}

void del(GTPTR &T,GTPTR root,string s,vector<char> a,int &i,int &f){
  if(!T) return;

  int x;
  if(i<s.size()) x = key(s[i],a);
  else if(i==s.size()) x = a.size();

  if(!T->u.ptr[x]->leaf) {del(T->u.ptr[x],root,s,a,++i,f);}
  else T->u.ptr[x] = NULL;

  if(T != root){
    if(T->u.ptr[x] && !T->u.ptr[x]->leaf && chi(T->u.ptr[x],a.size()) > -1 && f==1){
      T->u.ptr[x] = T->u.ptr[x]->u.ptr[chi(T->u.ptr[x],a.size())];
    }
    else f=0;
  }
}
